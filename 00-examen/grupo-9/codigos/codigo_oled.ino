#include <Wire.h>
#include <Adafruit_GFX.h>
#include <Adafruit_SSD1306.h>
#include "AdafruitIO_WiFi.h"

#define WIFI_SSID    "TU_WIFI"
#define WIFI_PASS    "TU_CLAVE"
#define IO_USERNAME  "TU_USUARIO"
#define IO_KEY       "TU_KEY"


#define TIEMPO_REPOSO 5000      // milisegundos sin datos hasta volver a reposo


// OLED

#define SCREEN_WIDTH   128
#define SCREEN_HEIGHT  64
#define SCREEN_ADDRESS 0x3C

Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, -1);


// ADAFRUIT IO

AdafruitIO_WiFi io(IO_USERNAME, IO_KEY, WIFI_SSID, WIFI_PASS);

AdafruitIO_Feed *pruebaMicro = io.feed("prueba-micro"); // recibe intensidad
AdafruitIO_Feed *cerebro     = io.feed("cerebro");      // envia ultimo digito a Arduino 1

//Frases

const char* respuestas[10] = {
  "SI",
  "NO",
  "QUIZA",
  "JAMAS",
  "OBVIO",
  "DUDOSO",
  "CLARO",
  "INTENTA",
  "ESPERA",
  "SEGURO"
};

// BITMAPS

const unsigned char epd_bitmap_frames_01[] PROGMEM = {
  
};

const unsigned char epd_bitmap_frames_02[] PROGMEM = {
  
};

const unsigned char epd_bitmap_frames_03[] PROGMEM = {
  
};

const unsigned char* frames[] = {
  epd_bitmap_frames_01,
  epd_bitmap_frames_02,
  epd_bitmap_frames_03
};
const int TOTAL_FRAMES = 3;

// ESTADO

enum EstadoPantalla { REPOSO, ANIMACION };
EstadoPantalla estadoPantalla = REPOSO;

unsigned long ultimoFrame        = 0;  // para el timer del gif
unsigned long ultimoMensaje      = 0;  // para el timer de reposo
int           frameActual        = 0;

// PROTOTIPOS

void recibirMicrofono(AdafruitIO_Data *data);
void mostrarReposo();
void actualizarAnimacion();
void verificarReposo();


// SETUP

void setup() {
  Serial.begin(115200);


  if (!display.begin(SSD1306_SWITCHCAPVCC, SCREEN_ADDRESS)) {
    Serial.println(F("Error: pantalla SSD1306 no encontrada"));
    while (true);
  }

  mostrarReposo();

  pruebaMicro->onMessage(recibirMicrofono);

  Serial.print("Conectando a Adafruit IO");
  io.connect();
  while (io.status() < AIO_CONNECTED) {
    Serial.print(".");
    delay(500);
  }
  Serial.println();
  Serial.println(io.statusText());

  pruebaMicro->get(); // pedir el ultimo valor conocido
}


// LOOP

void loop() {
  io.run();

  if (estadoPantalla == ANIMACION) {
    actualizarAnimacion();
    verificarReposo();   // vuelve a quieto si no llega nada en 5 seg
  }
}


// CALLBACK: llega intensidad del microfono
// Calcula ultimo digito → envia a "cerebro"
// Enciende LED + activa animacion

void recibirMicrofono(AdafruitIO_Data *data) {
  int intensidad   = data->toInt();
  int ultimoDigito = abs(intensidad) % 10;

  Serial.print("Intensidad: ");
  Serial.print(intensidad);
  Serial.print("  →  ultimo digito: ");
  Serial.println(ultimoDigito);

  cerebro->save(ultimoDigito);   // Arduino 1 lo lee para reproducir audio



  // activa animacion desde el frame 1
  estadoPantalla = ANIMACION;
  frameActual    = 0;
  ultimoMensaje  = millis();     // reinicia el contador de inactividad
}


// Imagen fija de reposo (frame 1)

void mostrarReposo() {
  display.clearDisplay();
  display.drawBitmap(0, 0, epd_bitmap_frames_01, SCREEN_WIDTH, SCREEN_HEIGHT, SSD1306_WHITE);
  display.display();
}


// Anima los 3 frames cada 120 ms

void actualizarAnimacion() {
  if (millis() - ultimoFrame < 120) return;
  ultimoFrame = millis();

  display.clearDisplay();
  display.drawBitmap(0, 0, frames[frameActual], SCREEN_WIDTH, SCREEN_HEIGHT, SSD1306_WHITE);
  display.display();

  frameActual++;
  if (frameActual >= TOTAL_FRAMES) frameActual = 0;
}


// Si pasan 5 seg sin nuevo dato → reposo

void verificarReposo() {
  if (millis() - ultimoMensaje >= TIEMPO_REPOSO) {
    estadoPantalla = REPOSO;

    mostrarReposo();
    Serial.println("Sin actividad → reposo");
  }
}
