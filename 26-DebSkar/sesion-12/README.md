# sesion-12

### lunes 01 junio 2026

La clase comenzó con el profesor mostrándonos un libro que está desarrollando para enseñar programación, diseñado especialmente para preparar a futuros estudiantes de este curso. Durante esta introducción, nos recalcó una idea clave:

* “Debemos aprender a explicar de forma clara nuestros proyectos y habilidades" *

Luego, pasamos a hablar detalladamente sobre los requisitos y la preparación para el examen final.

### Preparación de examen: Estudios Inalámbricos

El examen se abordará en grupos de 3 o 4 personas y consistirá en el desarrollo de un proyecto técnico-artístico.

Como parte de la entrega, hoy mismo se debe definir un párrafo de 4 líneas que explique claramente qué hace el proyecto, enfocándonos en el propósito y no en el cómo lo haremos.

Los criterios de evaluación del examen se dividen en dos ámbitos:

#### Evaluación Individual:

* Investigación de APIs: Cada integrante debe realizar una investigación individual sobre el uso e integración de APIs.

* Bitácora de proceso: Registro personal del avance y desarrollo (2 puntos).

#### Evaluación Grupal:

* Demo en video: Presentar una demostración grabada del funcionamiento entre los puntos de República 180 y SS221.

* Funcionamiento en vivo: Demostración en tiempo real funcionando de extremo a extremo de la sala.

* Documentación: Código ordenado y documentación estructurada de forma clara.

* Uso obligatorio de arrays.

* Utilizar al menos un microcontrolador.

* Utilizar al menos un sensor y/o un actuador.

#### Propuesta de Nuestro Proyecto

Somos el grupo 3: Josefa Araya, Débora Soto, Cristóbal Vergara, Nicolás Valdés.

#### Título del proyecto: Bruma Compacta

#### Descripción del Proyecto:

Nuestra idea inicial partió de la base de utilizar una API del clima para que, dependiendo de la temperatura, se reprodujera una canción. Sin embargo, la propuesta mutó al querer integrar un humidificador.

Ahora el proyecto busca expresar y materializar el clima a través de un humidificador alimentado por los datos de humedad ambiental obtenidos en tiempo real mediante una API.

El objetivo es provocar una escena inmersiva entre el agua y el usuario a través de una demostración literal y concentrada de la humedad.

#### Funcionamiento:

Para la interacción, el sistema seleccionará un lugar aleatorio de Chile a través de la API. Entonces cada vez que una persona interactúe con el proyecto, este lugar cambiará, provocando que el humidificador expulse una bruma de agua cuya intensidad y comportamiento responderán directamente a las condiciones climáticas de ese punto geográfico variable.


Por último, tuvimos que compartir la descripción breve de nuestro proyecto por Discord para recibir comentarios y preguntas de otros grupos. 

> josefa-araya [06/01/2026 4:48 PM]: Grupo 3: bruma compacta: Queremos expresar y materializar el clima a través de un humidificador alimentado de los datos de humedad en el ambiente (datos de un API), provocando una escena inmersiva entre el agua y el usuario. 

> montoyamora... [06/01/2026 4:50 PM]: ¿Cuál de todos los múltiples datos del clima? Ah, después dice humedad, entonces partir con humedad, filo con lo otro. (editado)

> josefa-araya [06/01/2026 4:50 PM]: La humedad.

> montoyamora... [06/01/2026 4:51 PM]: @josefa-araya por favor revisar el proyecto final del curso dis8645 del semestre pasado de @braulio-figueroa.
josefa-araya [06/01/2026 4:51 PM]: ¡Gracias!

> benjamin-alvarez [06/01/2026 4:53 PM]: Cuando dicen en el ambiente, ¿sería el ambiente donde están físicamente? ¿O un lugar random que la API les haya dado? ¿A qué se refieren con materializar el clima? (editado)

> josefa-araya [06/01/2026 4:55 PM]: De un lugar aleatorio pero de Chile.

> montoyamora... [06/01/2026 4:55 PM]: ¿Ese lugar aleatorio es siempre el mismo? ¿O cambia cada vez que alguien interactúa con el proyecto?

> josefa-araya [06/01/2026 4:56 PM]: Cambiaría cada vez.

> josefa-araya [06/01/2026 5:02 PM]: Con materializar el clima nos referíamos a que sería una demostración más literal y concentrada de la humedad al hacer que el humidificador expulse una bruma de agua. 

